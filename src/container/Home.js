...
export default (props) => {
  ...
  const [filterYear, setFilterYear] = useState(filterValues.years);
  const [filterLaunch, setFilterLunch] = useState(filterValues.launch);
  const [filterLanding, setFilterLanding] = useState(filterValues.landing);
  ...
  const fetchData = async () => {
    ...
    let result = await Axios(
      `https://api.spacexdata.com/v3/launches?limit=100${
        selectedFilter.launch === true || selectedFilter.launch === false
  ...
  return (
    <main className="row">
      ...
      {isLoading || !launchData ? (
        ...
        <article className="col-md-9 row">
          <LaunchList lunch_details={launchData} />
        </article>
      )}
    </main>
  );
